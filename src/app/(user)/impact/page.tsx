...
import { ArrowRight, LocateFixed, MapPin, QrCode, Trash2 } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        <ImpactStat Icon={MapPin} label="Thùng đang hoạt động" value={bins.length.toLocaleString("vi-VN")} />
        <ImpactStat Icon={LocateFixed} label="Sắp xếp theo vị trí" value="Gần nhất" />
        <ImpactStat Icon={QrCode} label="QR riêng từng thùng" value="Không trùng" />
      </section>
...
function ImpactStat({ Icon, label, value }: { Icon: typeof MapPin; label: string; value: string }) {
  return (
    <div className="rounded-[24px] border border-[#d9e5da] bg-white/88 p-4 shadow-[0_8px_24px_rgba(21,29,24,0.04)]">
      <span className="grid size-11 place-items-center rounded-2xl bg-[#d8f5df] text-[#007a3d]">
        <Icon size={20} />
      </span>
      <p className="mt-4 text-2xl font-black tracking-[-0.04em] text-[#151d18]">{value}</p>
      <p className="mt-1 text-xs font-bold leading-5 text-[#667468]">{label}</p>
    </div>
  );
}
