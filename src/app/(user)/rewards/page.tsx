...
export default function RewardsPage() {
  ...
  return (
    <div>
      <PageHeader eyebrow="Đổi thưởng" title="Phần thưởng xanh" body={`Bạn đang có ${user.points} điểm xanh. Chọn phần thưởng phù hợp để đổi ưu đãi từ hệ thống.`} />
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {rewards.map((reward) => {
          const canRedeem = user.points >= reward.pointsRequired;
          return (
            <article className="eco-card rounded-[32px] p-6" key={reward.id}>
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="rounded-full bg-[#e8fbff] px-3 py-1 text-sm font-black text-[#00687a]">{reward.pointsRequired} điểm</span>
              </div>
              ...
              <div className="mt-6 flex items-center justify-between rounded-2xl border border-[#e6e7ef] bg-white p-4">
                <div className="flex items-center gap-2">
                  <Leaf size={18} className="text-[#151515]" />
                  <span className="text-sm font-bold text-[#5f6472]">Còn hàng</span>
                </div>
                <Gift size={18} className="text-[#00687a]" />
              </div>
